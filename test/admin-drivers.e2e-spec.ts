...
describe('Admin Drivers E2E', () => {
  ...
  beforeAll(async () => {
    ...
    if (![200, 201].includes(createRes.status)) {
      throw new Error(`Driver create failed: ${createRes.status} ${JSON.stringify(createRes.data)}`);
    }
  });
  ...
  it('GET /admin/drivers returns filtered driver list', async () => {
    ...
    if (res.status !== 200) {
      throw new Error(`GET /admin/drivers failed: ${res.status} ${JSON.stringify(res.data)}`);
    }
    ...
    const matched = res.data.drivers.find((driver: any) => driver.phone === seededDriverPhone);
