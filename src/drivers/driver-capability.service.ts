...
export class DriverCapabilityService {
  ...
  async checkDeliveryAcceptanceCapability(driverId: string): Promise<CapabilityCheck> {
  ...
  private async buildConstraints(driver: Driver): Promise<CapabilityCheck['constraints']> {
    // Get active delivery count from Redis for real-time accuracy
    const activeDeliveries = await this.redisService.getClient().hget(
    ...
    return {
      ...
      requiresVehicleVerification: driver.vehicleType === 'BIKE' || driver.vehicleType === 'CAR',
      requiresBackgroundCheck: true, // Always required for production
      zoneRestricted: !!driver.zoneId,
    };
  }

  private checkActiveStatus(driver: Driver): { passed: boolean; reason?: string } {
    if (!driver.isActive) {
  ...
  private checkCurrentStatus(driver: Driver): { passed: boolean; reason?: string } {
  ...
  private async checkConcurrentDeliveryLimit(
    ...
    if (constraints.currentActiveDeliveries >= constraints.maxConcurrentDeliveries) {
      return { passed: false, reason: 'MAX_CONCURRENT_DELIVERIES_REACHED' };
    }
    return { passed: true };
  }

  private checkZoneAvailability(driver: Driver): { passed: boolean; reason?: string } {
    // Check if driver's zone is currently serviceable (e.g., not under curfew)
    // This would integrate with zone service
    return { passed: true };
  }

  private checkDocumentExpiry(driver: Driver): { passed: boolean; reason?: string } {
  ...
  async canAcceptSpecificDelivery(
    ...
    const baseCapability = await this.checkDeliveryAcceptanceCapability(driverId);
    ...
    const rejectionCount = await this.redisService.getClient().hget(
      `delivery:${deliveryId}:rejections`,
      driverId,
    );
