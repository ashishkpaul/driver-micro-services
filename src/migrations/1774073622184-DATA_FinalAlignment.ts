...
/**
 * INTENT:    Phase 44 of 47 — Column "user_id" requires backfill before NOT NULL can be enforced
 * TYPE:      DATA
 * RISK:      MEDIUM
 * ROLLBACK:  SAFE
 * *
...
export class DATAFinalAlignment1774073622184 implements MigrationInterface {
  name = 'DATAFinalAlignment1774073622184';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      -- Backfill: set a safe default for every existing row before enforcing NOT NULL
UPDATE import SET "user_id" = <DEFAULT_VALUE> WHERE "user_id" IS NULL;
    `);
  }
