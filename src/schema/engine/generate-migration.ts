...
function buildMigrationFile(
  ...
  const upStatements = sqlStatements
    .map((sql) => `    await queryRunner.query(\`\n      ${sql}\n    \`\);`)
    .join("\n\n");
...
export function writeCompanionMigrations(
  ...
  companions.forEach((companion, index) => {
    ...
    const timestamp = String(
      parseInt(path.basename(basePath).split("-")[0], 10) + index + 1,
    );
...
export function generateLifecycleMigrations(
  ...
  if (lifecycleSet.safe.length > 0) {
    const header = buildLifecycleHeader(
      "SAFE",
      name,
      "Expand",
      lifecycleSet.safe.length,
    );
    const filePath = writeMigrationFile(
      outputDir,
      "SAFE_",
      name,
      lifecycleSet.safe.map((op) => op.sql),
      header,
    );
  ...
  if (lifecycleSet.data.length > 0) {
    const header = buildLifecycleHeader(
      ...
      name,
      "Migrate",
      lifecycleSet.data.length,
    );
    const filePath = writeMigrationFile(
      outputDir,
      "DATA_",
      name,
      lifecycleSet.data.map((op) => op.sql),
      header,
    );
    if (filePath) createdFiles.push(filePath);
  }
  ...
  if (lifecycleSet.breaking.length > 0) {
    const header = buildLifecycleHeader(
      ...
      name,
      "Contract",
      lifecycleSet.breaking.length,
    );
    const filePath = writeMigrationFile(
      outputDir,
      "BREAKING_",
      name,
      lifecycleSet.breaking.map((op) => op.sql),
      header,
    );
    if (filePath) createdFiles.push(filePath);
  }
  ...
  if (companions.length > 0) {
    const companionFiles = writeCompanionMigrations(
      createdFiles[0] || "",
      companions,
      outputDir,
    );
