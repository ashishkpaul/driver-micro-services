...
import {
  detectSchemaDiff,
  detectSchemaDiffWithTypes,
} from "../migration-engine/detect-schema-diff";
import {
  classifyMigrationFile,
  classifySqlStatement,
} from "../migration-engine/classify-operations";
...
import { MigrationLifecycleSet } from "../migration-engine/types";
...
async function runGenerate(opts: {
  ...
  const rootCheck = require("child_process").spawnSync(
    "npx",
    ["ts-node", "scripts/project-root.ts"],
    { silent: true },
  );
  ...
  try {
    ...
    process.stdout.write(fmt.err("") + "\n");
    console.error(
      fmt.err("Schema detection failed:"),
  ...
  const files = fs
    ...
    .filter(
      (f: string) => f.includes(`TEMP_${strippedName}`) && f.endsWith(".ts"),
    )
    .sort()
    .reverse();
  ...
  try {
    ...
    const sqlStatements =
      require("../migration-engine/detect-schema-diff").extractSqlStatements(
        content,
      );
    ...
    const phases = splitter.split(sqlStatements, typeMap);

    // Convert PhasePlan[] to MigrationLifecycleSet
    const lifecycleSet: MigrationLifecycleSet = {
      safe: phases.find((p) => p.phase === "EXPAND")?.operations ?? [],
      data: phases.find((p) => p.phase === "DATA")?.operations ?? [],
      breaking: phases.find((p) => p.phase === "CONTRACT")?.operations ?? [],
    };
    ...
    const createdFiles = generateLifecycleMigrations(
      outputDir,
      strippedName,
      lifecycleSet,
      companions,
    ...
    createdFiles.forEach((f, i) => {
      ...
      const phase =
        i === 0 ? "Phase 1 (primary)" : `Phase ${i + 1} (companion)`;
    ...
    process.stdout.write(fmt.err("") + "\n");
    console.error(
      fmt.err("Lifecycle splitting failed:"),
      error instanceof Error ? error.message : String(error),
    );
    process.exit(1);
  }
  ...
  try {
    ...
    process.stdout.write(fmt.err("") + "\n");
    console.error(
      fmt.err("Governance validation failed:"),
      error instanceof Error ? error.message : String(error),
    );
    process.exit(1);
  }
  ...
  console.log(
    `  2. ${c.bold}Fill in the INTENT field${c.reset} in each file's header`,
  );
  console.log(`  3. ${c.bold}Run:${c.reset} npm run db:migrate`);

  // Check for BREAKING migrations
  const hasBreaking = fs
    .readdirSync(outputDir)
...
async function main(): Promise<void> {
  ...
  if (!args[0]) {
    ...
    console.error(
      `  Usage: ${c.bold}npm run migrate:generate -- <name>${c.reset}`,
    );
    process.exit(1);
  }
