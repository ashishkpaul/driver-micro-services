/**
 * scripts/db/baseline.ts  — FIX v4
 *
 * CHANGES IN THIS VERSION:
 *
 * Fix 3 — Enum idempotency:
 *   `CREATE TYPE ... AS ENUM` has no IF NOT EXISTS syntax in PostgreSQL.
 *   If baseline is re-run against a partially-initialised database (e.g. a crash
 *   mid-migration, or re-running db:setup after a schema:drop left enums behind),
 *   every enum creation throws "type already exists" and aborts the migration.
 *   Fixed: each enum is wrapped in:
 *     DO $$ BEGIN CREATE TYPE ...; EXCEPTION WHEN duplicate_object THEN NULL; END $$;
 *
 * Fix 1 (baseline copy) — Correct index names:
 *   The previous baseline created idx_delivery_pending on BOTH delivery_events
 *   and driver_offers. The second silently no-oped (schema-level name collision).
 *   Fixed: driver_offers now uses idx_driver_offers_delivery_id.
 *   Note: The live database needs the incremental migration
 *   1753920000000-SAFE_FixDuplicateDeliveryPendingIndex.ts applied separately.
 *
 * Fix 1 (baseline copy) — Correct column types for drivers:
 *   drivers.city_id and zone_id were VARCHAR; corrected to UUID to match
 *   cities.id and zones.id column types.
 *   Note: The live database needs the incremental migration
 *   1753920001000-BREAKING_FixDriversCityZoneUUID.ts applied separately.
 */

import * as fs   from 'fs';
import * as path from 'path';

const BASELINE_TIMESTAMP = '1000000000000';

// ── Idempotent enum helper ────────────────────────────────────────────────────
// PostgreSQL has no CREATE TYPE IF NOT EXISTS. This DO block swallows
// duplicate_object errors so the baseline is safe to replay.

function createEnumSql(typeName: string, values: string[]): string {
...
const SCHEMA_UP_BODY = `
    // ── 1. Enums (idempotent — safe to replay) ────────────────────────────────

    await queryRunner.query(\`${createEnumSql('driver_status_enum', ['AVAILABLE','BUSY','OFFLINE'])}\`);

    await queryRunner.query(\`${createEnumSql('admin_role_enum', ['DRIVER','ADMIN','DISPATCHER','OPS_ADMIN','CITY_ADMIN','SUPER_ADMIN','SYSTEM'])}\`);

    await queryRunner.query(\`${createEnumSql('outbox_status_enum', ['PENDING','PROCESSING','COMPLETED','FAILED'])}\`);

    await queryRunner.query(\`${createEnumSql('delivery_status_enum', ['PENDING','ASSIGNED','PICKED_UP','IN_TRANSIT','DELIVERED','FAILED','CANCELLED'])}\`);

    await queryRunner.query(\`${createEnumSql('delivery_event_type_enum', ['ASSIGNED','PICKED_UP','IN_TRANSIT','DELIVERED','FAILED','CANCELLED'])}\`);

    await queryRunner.query(\`${createEnumSql('driver_offer_status_enum', ['PENDING','ACCEPTED','REJECTED','EXPIRED'])}\`);

    await queryRunner.query(\`${createEnumSql('notification_method_enum', ['push','websocket','both'])}\`);

    await queryRunner.query(\`${createEnumSql('auth_provider_enum', ['legacy','google','email'])}\`);
    ...
    // ── 5. drivers (city_id/zone_id are UUID — cross-service logical refs) ────
    ...
    await queryRunner.query(\`CREATE INDEX IF NOT EXISTS idx_delivery_events_delivery_event  ON delivery_events (delivery_id, event_type)\`);
    await queryRunner.query(\`CREATE INDEX IF NOT EXISTS idx_delivery_events_seller_order_id ON delivery_events (seller_order_id)\`);
    // idx_delivery_pending is the canonical index for delivery_events.delivery_id
    await queryRunner.query(\`CREATE INDEX IF NOT EXISTS idx_delivery_pending ON delivery_events (delivery_id)\`);
    ...
    await queryRunner.query(\`CREATE INDEX IF NOT EXISTS idx_assignments_seller_driver ON assignments (seller_order_id, driver_id)\`);
    ...
    // FIXED: unique names — idx_driver_offers_delivery_id, not idx_delivery_pending
    await queryRunner.query(\`CREATE INDEX IF NOT EXISTS idx_driver_offers_delivery_id ON driver_offers (delivery_id)\`);
...
export function ensureBaselineExists(): void {
  ...
  const fileName  = `${BASELINE_TIMESTAMP}-BASELINE_Initial.ts`;
  const className = `BASELINE_Initial${BASELINE_TIMESTAMP}`;
...
export class ${className} implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {${SCHEMA_UP_BODY}  }

  public async down(queryRunner: QueryRunner): Promise<void> {${SCHEMA_DOWN_BODY}  }
}
`;
