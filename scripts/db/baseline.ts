/**
 * scripts/db/baseline.ts  — FIX v3
 *
 * ROOT CAUSE OF THIS FIX:
 *   The baseline migration generated by earlier versions contained only
 *   `queryRunner.query('SELECT 1')` — a no-op marker. It recorded itself in
 *   `_migrations` but created zero schema objects. Every table, index, enum,
 *   and constraint was missing. db:verify reported 26 failures.
 *
 * FIX:
 *   `ensureBaselineExists()` now generates a migration whose `up()` builds
 *   the complete driver microservice schema:
 *     • 8 PostgreSQL enums
 *     • 11 tables (cities, zones, admin_users, drivers, deliveries,
 *       delivery_events, assignments, driver_offers, outbox, outbox_archive,
 *       audit_logs)
 *     • All indexes required by db-verify.ts
 *     • All foreign keys required by db-verify.ts
 *
 *   The `down()` is a full teardown (safe for dev/CI; never run in production).
 *
 * DESIGN RULES preserved from prior fixes:
 *   • BASELINE_TIMESTAMP = '1000000000000' (non-zero, sorts before dev migrations)
 *   • `name` field is explicitly set (TypeORM reads it from the instance)
 *   • File + class name are deterministic across all environments
 */

import * as fs   from 'fs';
import * as path from 'path';

// Fixed anchor timestamp — September 9, 2001.
// Non-zero, 13 digits, sorts before any modern (2024+) development migration.
const BASELINE_TIMESTAMP = '1000000000000';

// ── Schema DDL ────────────────────────────────────────────────────────────────
// Inlined here so the generated migration is fully self-contained.
// If you add a new entity, generate a new incremental SAFE_ migration instead
// of editing this baseline.
